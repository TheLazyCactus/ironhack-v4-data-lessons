...
-- Limiting the result set (use the brush to make the code pretty) --
SELECT 
...
SELECT DISTINCT A3  AS Region FROM bank.district;
SELECT DISTINCT district_name FROM bank.district ORDER BY A2 DESC; 
...
ORDER BY A2 DESC
LIMIT 30; 
-- Order is relevant and have to rename at all time as not updating the data just checking --
-- ==================================================
...
SELECT * FROM bank.loan
WHERE status IN ('B','b') AND NOT amount > 100000;
SELECT * FROM bank.loan
WHERE status IN ('B','b') OR  amount > 100000;
...
SELECT 
    account_id, loan_id, amount - payments AS debt
FROM
    loan
WHERE
    status IN ('B')
...
WHERE
    A10 <= 50
ORDER BY A10 DESC;
-- Limit asways at the end of the call --
-- ==================================================
...
SELECT COUNT(*) FROM order; -- will not work as SQL is taking order as a command
SELECT * FROM bank.`order`; -- Use it as a last solution is dt.order not working
...
-- We can concatenate two strings using the function `concat()`= to merge column
...
-- Functions to drop white spaces of a string `ltrim()` and `rtrim()` = left trim or right trim
SELECT LTRIM(' Hello ');
SELECT RTRIM(' Hello ');
...
SELECT account_id, district_id, frequency, 
       CONVERT(account.date, DATE) AS date_formatted 
FROM bank.account;
...
SELECT CONVERT (issued, DATE)
FROM bank.card; 
-- Null as not the right datatype

SELECT SUBSTRING_INDEX(issued, ' ', 1) 
FROM bank.card;
-- Removed the extra 00
...
-- Exact format depends of the language can be yy mm dd
...
WHERE k_symbol IS NULL; -- blank space are treated as NULL value 
...
CASE 
    WHEN status = 'A' THEN 'Good - Contract Finished' -- Like if then
...
CASE 
    WHEN amount < 50000 THEN 'Small' -- Like if then
    WHEN amount BETWEEN 50000 AND 150000 THEN 'Medium'
    WHEN  amount BETWEEN 150000 and 500000 THEN 'Large'
    ELSE 'Platinium'
...
CASE 
    WHEN amount < 50000 THEN 'Small' -- Like if then
    WHEN amount BETWEEN 50000 AND 150000 THEN 'Medium'
    WHEN  amount BETWEEN 150000 and 300000 THEN 'Large'
    ELSE 'Platinium'
END AS 'Loan_Category', -- Create a new column
COUNT(loan_id)
FROM bank.loan
...
FROM bank.loan
GROUP BY Loan_Category
HAVING Loan_Category <> 'Platinium' 
-- Where is only working on raw data so cannot use it for loan category. Having is using the new column as well
;
...
CASE 
    ...
    WHEN amount BETWEEN 100000 AND 200000 THEN 'Medium - Large'
    WHEN  amount BETWEEN 200000 and 300000 THEN 'Large'
    ELSE 'Platinium'
END AS 'Loan_Category', -- Create a new column
...
-- 1. Get the total lending revenue using the column `amount` from table `payment`. Display it as `Total_revenue`. 
SELECT
	SUM(amount) AS 'Total_revenue'
    FROM loan;
    
-- 2. Get the total lending revenue by customer_id sorted by total benefit in descending order.
SELECT
...
WHERE A2 LIKE 'K';
-- Finding district names starting with 'K', % is saying not perfect match
SELECT * FROM bank.district
WHERE A2 LIKE 'K%';
...
SELECT 
    DATE_FORMAT(CONVERT( date , DATE), '%Y') AS year, -- unaggregated column
    status,
