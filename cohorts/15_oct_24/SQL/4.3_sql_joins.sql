...
LEFT JOIN account ON loan.account_id = account.account_id; -- 682

SELECT * FROM loan
RIGHT JOIN account ON loan.account_id = account.account_id;

SELECT COUNT(*) FROM loan
RIGHT JOIN account ON loan.account_id = account.account_id; -- 4.5K

SELECT COUNT(*) FROM loan
JOIN account ON loan.account_id = account.account_id; -- 682 = minimum between the 2 joints

SELECT COUNT(*) FROM loan
FULL JOIN account USING (account_id); -- Inner joint as well 682 = minimum between the 2 joints
...
-- self join good if 2 columns with the same meaning
...
-- Give the database alias d1/d2
-- Precise which database A2 is coming from
