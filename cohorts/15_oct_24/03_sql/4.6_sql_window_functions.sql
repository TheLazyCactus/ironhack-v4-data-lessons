...
-- will give the average amount per status 
SELECT status,
       loan_id, 
       duration, 
       amount,
...
-- Different from ROw where row will only start again when change group where rank will keep the same rank if the rows are the same but start at +1
...
-- DENSE RANK will make you pass to the next number after a tie instead of jumping a number
SELECT status,
       loan_id, 
       duration, 
       amount,
       AVG(amount) OVER(PARTITION BY status, duration) AS 'Avg_amount',
       ROW_NUMBER() OVER(PARTITION BY status, duration ORDER BY status, duration, amount DESC) AS 'Row_number',
       RANK() OVER(PARTITION BY status, duration ORDER BY status, duration, amount DESC) AS 'Rank',
       DENSE_RANK () OVER(PARTITION BY status, duration ORDER BY status, duration, amount DESC) AS 'Desc_Rank'
FROM bank.loan
ORDER BY status, duration, amount DESC;

-- VIEWS
CREATE VIEW  window_amount AS
SELECT status,
       loan_id, 
       duration, 
       amount,
...
-- Access the view by calling the name with SELECT
...
-- cte view just for that specific query
WITH cte_view AS (
  SELECT 
   Activity_year, 
   Activity_month,
